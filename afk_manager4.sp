...
public Action Timer_CheckPlayer(Handle Timer, int client) // General AFK Timers
{
	...
	if(g_bEnabled) // Is the AFK Manager Enabled
	{
...
#endif
		if ((Synergy) || ((g_iPlayerTeam[client] != 0) && (g_iPlayerTeam[client] == 2))) // Make sure player is not Unassigned or Spectator
...
#endif
		if (g_iPlayerTeam[client] == 2) // Check we are not on the Spectator team
		{
		...
		if (iKickPlayers > 0)
			...
			{
				if ((iKickPlayers == 2) && (g_iPlayerTeam[client] != 2)) // Kicking is set to exclude spectators. Player is on the spectator team. Spectators should not be kicked.
					return Plugin_Continue;
				else
				{
